...
def update_province_map(selected_provinces, clicked_markers, all_provinces):
    if not unzipped:
    ...
    try:
        ...
        if not filtered_gdf.empty:
            ...
            # Create figure with selected provinces
            fig = px.choropleth_mapbox(
                filtered_gdf,
            ...
            for province in selected_provinces:
                for poi in hardcoded_poi_coordinates.get(province, []):
                    # Determine marker color based on whether it's been clicked
                    marker_color = 'green' if poi['marker_id'] in clicked_markers else 'red'
